...
function Datepicker() {
		...
		yearRange: '-70:+5', // Range of years to show in drop-down: 'any' for direct text entry
	...
	selectDate: function(target, elem) {
		...
		if (inst && !this.isDisabled(target)) {
			...
			if (inst.inline || inst.pickingRange || inst.selectedDates.length <
                ...
                var getmode = $('#modeval').val();
                var moreautoval = $('#moreautoval').val();
                if(getmode=='common') getDateval(); // customise function
                else if(getmode=='more') getmoreDateval(moreautoval);
	},
